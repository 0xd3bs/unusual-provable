...
fn jacobi_eigensystem(
    ...
    loop {
        ...
        let theta = if (a)
            ...
            // The denominator is computed first
            let denominator = FP16x16Sub::sub(
                (a).at(indices: array![p, p].span()), (a).at(indices: array![q, q].span())
            );
            // We define a small epsilon (2^-8 = 1/256)
            let epsilon = FixedTrait::<FP16x16>::new(256, false);

            // If the denominator is very close to zero, we use pi/4
            if FixedTrait::abs(denominator) < epsilon {
                FP16x16Div::div(pi, four)
            } else {
                half
                    * trig::atan(
                        FP16x16Div::div(two * (a).at(indices: array![p, q].span()), denominator)
                    )
            }
        };
